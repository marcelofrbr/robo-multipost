...
| `src/public-api/` | Versioned public API (`/v1/`), authenticated by API key. `POST /upload` and `POST /upload-from-url` link the created media to `publicApiProfileId` (profile-scoped API key) and return `{ id, path }` (not the full Media object). `upload-from-url` delegates to `MediaService.uploadFromUrl`. `routes/v1/public.flows.controller.ts` covers automations + DM bot/escalations under `/public/v1/flows`, with its own strict `ValidationPipe({ whitelist, forbidNonWhitelisted, transform })`. Profile-scope resolution for **all four** public controllers goes through the shared `PublicApiScopeService.resolveProfileId(orgId, keyProfileId, requestedProfileId)` (403 when a profile-scoped key's `?profileId`/body diverges, 404 when an org-scoped key requests a `profileId` from another org) — inject it, never reimplement the check per controller. Write paths are `@Throttle`d and call `FlowsService.assertIntegrationAccess` (412 missing/disabled, 403 other profile). `public.posts.controller.ts` (post detail/group/statistics/date/comments) and `public.media.controller.ts` (library list/delete/information) follow the same pattern; channel enable/disable/settings and OAuth (`GET /social/:integration`, which now stashes `profile:${state}` in Redis so the callback lands the channel in the right profile) live in `public.integrations.controller.ts` — the route only delegates to `IntegrationService.createAuthUrl(orgId, provider, { profileId?, refresh? })`, the single OAuth-URL implementation shared with the MCP tool `integrationAuthUrl` (see [`libraries/nestjs-libraries/CLAUDE.md`](../../libraries/nestjs-libraries/CLAUDE.md)). Scope helpers are in the services: `PostsService.getPostInScope/getGroupInScope` (posts are strict per profile → 404), `MediaService.getMediaInScope` and `IntegrationService.getIntegrationInScope` (null profile = shared → 403 for another profile, 404 when missing). Posts and integrations returned by `public.posts.controller.ts`/`public.integrations.controller.ts` are sanitized by `toPublicPostPayload`/`toPublicIntegration` (`libraries/nestjs-libraries/src/database/prisma/posts/public.post.mapper.ts`) before leaving the process — never forward a raw `include: { integration: true }` row (token/refreshToken). |
...
8. **Symptom:** a `/public/v1` route accepting `profileId` (query or body) returns/mutates data belonging to a different profile than the one the API key is scoped to, or an org-scoped key persists a `profileId` from another organization onto a `Flow`/`Integration` → **Cause:** the caller-supplied `profileId` was not reconciled against the key's `publicApiProfileId` (profile-scoped key) or against the caller's org (org-scoped key) before delegating to the service. **Fix:** every new `/public/v1` route that accepts `profileId` MUST inject `PublicApiScopeService` (`libraries/nestjs-libraries/src/services/public-api-scope.service.ts`) and call `resolveProfileId(orgId, publicApiProfileId, requestedProfileId)` (403 on profile-key mismatch, 404 when an org-key requests a profile from another org) before calling the service — do not reimplement the check locally. Same IDOR-closing spirit as pitfall #7, but for profile scope instead of org scope.
9. **Symptom:** a `/public/v1` response leaks OAuth `token`/`refreshToken` (or other private fields) because it forwards the raw `integration` object embedded by a repository's `include: { integration: true }` → **Cause:** repositories built for the private dashboard return the full `Integration` row for convenience; a public controller returned it unfiltered. **Fix:** wrap the payload with `toPublicPostPayload`/`toPublicIntegration` from `libraries/nestjs-libraries/src/database/prisma/posts/public.post.mapper.ts` before returning it — allowlists `id/name/picture/providerIdentifier/disabled/profileId` (+ `customer.id/name`), the same fields as `GET /public/v1/integrations`.
