...
| `src/database/prisma/startup-migration.service.ts` | **Code-level data migrations** that run on every app startup (idempotent backfills, not schema changes). Five steps in `onModuleInit`: `migrateProfileScope`, `migrateLateToZernio`, `backfillRepostDestinations`, `cleanupExpiredUnmatchedComments`, `cleanupOldMedia`. See pitfall below. |
| `src/database/prisma/flows/flows.service.ts` | Flow (automation) business logic: CRUD, status, executions, IG targets (posts/stories), DM bot. `assertIntegrationAccess(orgId, integrationId, profileId?)` is the canonical guard for any caller-supplied `integrationId` (412 if missing/disabled, 403 if it belongs to another profile; `profileId: null` = shared integration, mirrors `IntegrationRepository`) — call it before touching the integration, same spirit as pitfall #7 in [`apps/backend/CLAUDE.md`](../../apps/backend/CLAUDE.md). |
| `src/database/prisma/integrations/integration.service.ts` | `createAuthUrl(orgId, provider, { profileId?, refresh? })` is the single OAuth-URL implementation for connecting a channel — shared by `GET /public/v1/social/:integration` (public API) and the MCP tool `integrationAuthUrl`; stashes `organization:`/`login:`/`profile:`/`refresh:` + `state` in Redis (1h TTL) for the callback. Any change to that flow (TTL, Redis keys) is made here once. |
| `src/database/prisma/flows/flows.repository.ts` | Prisma queries for `Flow`/`FlowExecution`. `getFlows(orgId, profileId?, integrationId?)` filters in the `where`; `getExecution(orgId, id, flowId?)`/`getExecutions` filter by `flow: { organizationId: orgId }` (and optionally `flowId`) to close IDOR on execution IDs. |
...
| `src/database/prisma/dm/dm.repository.ts` | Repositorio dos models `DmConversation`/`DmMessage` (enum `DmConversationStatus`) |
| `src/database/prisma/dm/dm-flow.service.ts` | Intake de DM: kill-switch via Flow `direct_message` ACTIVE, idempotencia por `metaMid`, rate limit, enfileira `dm-bot-reply.workflow.ts` |
| `src/database/prisma/dm/dm-bot.service.ts` | Geracao da resposta (RAG + persona + AiClientFactory); fail-safe (escala pra humano em vez de inventar) |
| `src/database/prisma/dm/dm-rate-limit.service.ts` | Rate limit via Redis (`DM_BOT_RATE_LIMIT_PER_HOUR`, `DM_BOT_MAX_REPLIES_PER_CONVERSATION`) |
| `src/database/prisma/dm/dm-window.helper.ts` | Guarda da janela de 24h da Meta |
...
6. **Symptom:** App starts cleanly after deploy but data looks wrong (rows with stale `lateApiKey`, `providerIdentifier='late-X'` instead of `zernio-X`, missing `RepostRuleDestination` rows, unexpected media not cleaned up, etc.) → **Cause:** `StartupMigrationService` (`OnModuleInit`) runs every startup with five idempotent steps (`migrateProfileScope`, `migrateLateToZernio`, `backfillRepostDestinations`, `cleanupExpiredUnmatchedComments`, `cleanupOldMedia`), but it **swallows errors**: failures are only logged, the app keeps running. **Fix:** tail the backend logs on deploy for `Late->Zernio migration failed:` / `Profile scope migration failed:` / `Backfill RepostRuleDestination falhou:` / `cleanupOldMedia falhou:`. Each method begins with a `count()` guard that skips when no rows need work, so re-running is safe: fix the underlying issue and restart the backend. New data migrations belong here when they need application-level logic (joins, format mapping, conditional updates). Pure DDL still goes through Prisma migrations.
7. **Symptom:** `PENDING UnmatchedComment` rows (or old gallery media) accumulate indefinitely after adding a new inbox-like or media-retention feature → **Cause:** time-windowed cleanup must be registered in `StartupMigrationService.onModuleInit`, not as a standalone `@Cron` service. Two canonical examples: `cleanupExpiredUnmatchedComments` (30-day comment retention) and `cleanupOldMedia` (delegates to `MediaCleanupService`, configurable via `MEDIA_RETENTION_DAYS`, default 30 days). Both are count-guard idempotent and swallow errors per the existing pattern. **Fix:** add an idempotent cleanup method to `StartupMigrationService` and call it from `onModuleInit`. Do NOT create a separate cron service.
8. **Symptom:** DM bot responde mesmo apos ser desligado, ou nao responde mesmo estando ligado → **Cause:** o kill-switch do bot de DM NAO e uma env var; e o status do Flow do tipo `direct_message` (ACTIVE = ligado, PAUSED = desligado). A env var `DM_BOT_RATE_LIMIT_PER_HOUR` e `DM_BOT_MAX_REPLIES_PER_CONVERSATION` controlam apenas o volume de respostas — sao independentes do kill-switch. **Fix:** para ligar/desligar o bot, use `POST /flows/dm/bot` (togglavel via MCP tool `configureDmBot` ou diretamente na UI). Para ajustar limites de volume, configure as env vars. Nunca tente controlar o bot via env var de feature flag.
9. **Symptom:** `GET /media` reports more pages than the list actually has content for (or a filtered page renders empty while `pages > 0`) → **Cause:** `count()` and `findMany()` built separate `where` objects; if they drift (e.g., `count` missing `deletedAt: null` or a new filter), soft-deleted/out-of-range rows inflate the page count. **Fix:** `MediaRepository.getMedia` builds a single `Prisma.MediaWhereInput` and reuses it for both `count` and `findMany`. Apply the same pattern to any new paginated repository method.
