...
- **Never** open or mutate a PR against `maiconramos/robo-multipost` or `gitroomhq/postiz-app`. **PRs always go to our own repository, `marcelofrbr/robo-multipost`** — this clone is a fork of a fork, and without `--repo` the `gh` CLI resolves PRs against the parent (PRs landed in the public Postiz repo, and PR #45 on maiconramos was auto-closed as spam by its `PR Quality` workflow). Enforced by hook `.claude/hooks/gh-pr-fork-guard.sh` (exit 2 on `gh pr <create|edit|merge|close|ready|review|comment|reopen>` without `--repo marcelofrbr/robo-multipost`, or with `--repo maiconramos/...`/`gitroomhq/...`). Always use `gh pr <subcommand> --repo marcelofrbr/robo-multipost --base main --head <branch> ...`. Read-only `gh pr view|list|diff|checks|status` are not blocked. Production images are built from this fork (`ghcr.io/marcelofrbr/robo-multipost:all`), never from maiconramos.
